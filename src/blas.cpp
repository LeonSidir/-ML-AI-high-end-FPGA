...
void strsm_hw(char side, char uplo, char transa, char diag, int m, int n,
           float alpha, float *A1, int lda, float *B1, int ldb) {
    ...
    if (uplo == 'U') {
        for (j = 0; j < n; j++) {
            for (k = m - 1; k >= 0; k--) {
                if (nounit) {
                    B1[j*ldb + k] = B1[j*ldb + k] / A1[k*lda + k];
                }
                for (i = 0; i < k; i++) {
                    B1[j*ldb + i] = B1[j*ldb + i] - B1[j*ldb + k] * A1[k*lda + i];
                }
            }
        }
    } else {
...
void sgemm_hw(char transa, char transb, int m, int n, int k,
           float alpha, float *A1, int lda, float *B1, int ldb,
           float beta, float *C1, int ldc) {
    int i, j, l;

    for (j = 0; j < n; j++) {
	    for (l = 0; l < k; l++) {
	        for (i = 0; i < m; i++) {
	            C1[j*ldc+i] += alpha*B1[j*ldb+l] * A1[l*lda+i];
	        }
	    }
    }
    return;
